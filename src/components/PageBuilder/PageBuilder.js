...
const PageBuilder = ({ structure, type, listProps, isCategory }) => {
  ...
  return Array.isArray(structure) && structure?.map((el, i) => {
    const cardMapping = {
    ...
    if (el.title === 'categories_advertisements') {
      ...
      return <div style={{ margin: '50px 0' }} key={i}>
    ...
    if (el.title === 'categories_recentlySold')
      ListElement = SoldProductCard
    ...
    const renderCards = el?.auctions?.map(cardEl => {
      const cardData = {
        ...
        price: cardEl?.vehiclePrice ?? (cardEl?.highestBidPrice || cardEl?.startingPrice),
