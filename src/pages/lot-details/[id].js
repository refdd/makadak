...
import { Alert, Button, Dialog, IconButton, Snackbar, TextField, useMediaQuery, useTheme } from '@mui/material'
...
import { useMakeOfferMutation, useWithdrawOfferMutation } from '@/redux/apis/auction-salesApi/buyerApi'
import { useAcceptSpecificOfferMutation, useCounterOfferMutation, useDeclineOfferMutation, useGetUserOffersQuery } from '@/redux/apis/auction-salesApi/sellerApi'
...
const LotDetails = ({ auctionDetails, category, highestBid, error }) => {
  ...
  const handleOpenOfferDialog = () => {
    if (user?.deposit?.amount === undefined)
      router.push('/auth')
  ...
  const enoughBalance = user?.deposit?.amount >= auctionDetails?.depositAmount?.amount
  ...
  const getOffersQ = useGetUserOffersQuery({ auctionVehicleId: auctionDetails?.id }, { skip: !showSellerOffers });
  ...
  const onPayForReport = () => {
    storeInspectionQ(auctionDetails?.id).unwrap()
      .then(res => {
  ...
  const getTime = () => {
    setDays(Math.floor(auctionDetails?.timeRemaining.secondsLeft / (60 * 60 * 24)));
    setHours(Math.floor((auctionDetails?.timeRemaining.secondsLeft / (60 * 60)) % 24));
    setMinutes(Math.floor((auctionDetails?.timeRemaining.secondsLeft / 60) % 60));
  };
  ...
  const propertiesMapping = [
    ...
    { title: 'Interior Condition', path: auctionDetails?.vehicleInteriorCondition },
  ...
  const renderProperties = propertiesMapping.map(({ title, path }, i) =>
     ((title || path) && ( path != null || path != ''))  && (
      <Grid item xs={6} md={3} key={i}>
        <Typography fontWeight={300} fontSize={13}
          sx={{ color: 'white' }}>{title}</Typography>
        <Typography
          fontWeight={600}>{path} </Typography>
      </Grid>
    )
  )

  const inspectionType = auctionDetails?.addOns?.inspectionReport?.categoryPriceType;
  ...
  const inspectionPrice = auctionDetails?.vehicleModel?.categories.length > 0 ?
  ...
  const handleBuyNow = () => {
    if (user?.deposit?.amount === undefined)
      ...
      if (enoughBalance)
        lockBuyQ(router.query.id).unwrap()
          .then(res => {
            if (res.error)
              setSnackbarState(state => ({
                ...state,
        ...
        router.push(`/wallet?type=${auctionDetails?.saleType}&id=${auctionDetails?.id}&amount=${auctionDetails?.depositAmount?.amount}`)
    }
  }

  const handleWithdrawOffer = () => {
    withdrawOfferQ({ "auctionVehicleId": auctionDetails?.id }).unwrap()
      .then(res => {
        setSnackbarState(state => ({
          type: 'success',
          open: true,
  ...
  const handleDeclineOffer = (id) => {
    declineOfferQ({ "auctionVehicleSaleOfferId": id }).unwrap()
      .then(res => {
        setSnackbarState(state => ({
          type: 'success',
          open: true,
  ...
  const handleCounterOffer = (id, amount) => {
    counterOfferQ({ id, amount: amount }).unwrap()
      .then(res => {
        setSnackbarState(state => ({
          type: 'success',
          open: true,
  ...
  const handleAcceptSpecificOffer = (id) => {
    acceptOfferQ({ auctionVehicleSaleOfferId: id }).unwrap()
      .then(res => {
        setSnackbarState(state => ({
          type: 'success',
          open: true,
    ...
    makeOfferQ({
      ...
      .then(res => {
        ...
        setSnackbarState(state => ({
          ...state,
          open: true,
          message: !!e?.data?.payload?.validation?.length ? e?.data?.payload?.validation[0]?.errors[0]?.message : e.data.message
        }))
  ...
  const handleCardClick = (type) => {
    if (type === 'stc') {
      stcPaymentQ({ amount: inspectionPrice, inspectionReportId, type: 'inspection_report_advanced_buyer' }).unwrap()
        .then((res) => {
          console.log(res);
  ...
  const renderOffers = getOffersQ?.data?.map((el, i) => {
    return <Box key={i} sx={{ padding: 2, borderRadius: 3, background: 'rgb(35, 35, 35)' }}>
      <Typography>Auction ID: &nbsp;
        <Typography sx={{ display: 'inline-block' }} fontWeight={600}>
          {el.auctionVehicleId}
        </Typography>
      </Typography>
      <Typography>Offer Amount: &nbsp;
        <Typography sx={{ display: 'inline-block' }} fontWeight={600}>
...

      {
        ...
        <Typography>Counter Offer: &nbsp;
          <Typography sx={{ display: 'inline-block' }} fontWeight={600}>
            {el.counterOfferAmount.currency.code} {el.counterOfferAmount.amount}
          </Typography>
        </Typography>
      }
      <Box>
        <Button onClick={() => handleAcceptSpecificOffer(el.id)}>Accept</Button>
      ...
      </Box>
      {
        ...
        <Box>
          ...
          />
          <Button onClick={() => handleCounterOffer(el.id, counterOfferAmount)}>Send Offer</Button>
    ...
    <Box sx={{ width: '90%', margin: '4% auto', paddingBottom: 10 }}>
      {
        isMobile ?
          ...
          :
          <>
            <CustomImageList itemData={auctionDetails?.mediaPhotos} handleOpen={handleOpen} />
            <GalleryModal
              open={open}
      ...
      <Grid container flex flexDirection={'column'} padding={1} my={2}>
        <Grid item xs={12}>
          <Grid container justifyContent={'space-between'}>
            ...
            <Grid item >
              <FavoritesButton id={auctionDetails?.id} isFavourite={auctionDetails?.isFavourite} />
            </Grid>
          </Grid>
          <Grid item>
              ...
              <Grid item xs={12} md={7}>
                <Grid container flexDirection={'column'}>
                  <Grid item my={2} xs={12}>
                    <Typography fontWeight={700} fontSize={22}>Lot Details</Typography>
                  </Grid>
                  <Grid item xs={12}>
                ...
                <Grid my={2} container flexDirection={'column'} justifyContent={'center'}>
                  <Grid item display={'flex'} alignItems={'center'}>
                    <LocationOnIcon />
                    <Typography fontSize={16} fontWeight={600}>Item Location</Typography>
                  </Grid>
                  <Grid item>
                    <Typography color='whitesmoke' fontWeight={300} fontSize={14}>
                      {
                ...
                {
                  inspectionPrice &&
                  <Grid my={2} spacing={2} container flexDirection={'column'} justifyContent={'center'}>
                    <Grid item display={'flex'} alignItems={'center'}>
                      <IconButton sx={{ background: '#00F0A9', marginRight: 2 }} size='small' >
                        <FindInPageIcon />
                      </IconButton>
                      <Typography fontWeight={600} fontSize={18}>Inspection Report</Typography>
                    </Grid>
                    <Grid item>
                      <Typography fontWeight={500}>
                        Get an additional inspection report and valuation for SAR {inspectionPrice}
                      </Typography>
                      <Typography fontWeight={300} color='grey' fontSize={14}>
                        Once you have made payment, an inspection Report for this lot will be available from your account section.
                      </Typography>
                    </Grid>
                    <Grid item display={'flex'} width={'50%'} justifyContent={'space-between'}>
                      <Typography>Buy an Inspection Report</Typography>
                      <IOSSwitch onChange={() => setInspectionReportChecked(state => !state)} value={inspectionReportChecked} />
                    </Grid>
                    {
                      inspectionReportChecked &&
                      <Grid item>
                        <Button variant='contained' size='small' sx={{ borderRadius: 2 }} onClick={onPayForReport}>
                          <Typography fontWeight={600} fontSize={14} color='black'>Pay for report</Typography>
                        </Button>
                      </Grid>
                    }
                  </Grid>
                ...
                <Grid container flexDirection={'column'} justifyContent={'space-between'} >
                  {
                    ...
                    <>
                      <Grid item>
                        <Typography textAlign={'center'} fontWeight={700} fontSize={26}>
                    ...
                    <Grid container flexDirection={'column'} justifyContent={'space-between'}>
                      {
                        ...
                        <Grid item fontWeight={400} fontSize={26} textAlign={'center'}>Lot
                          Price</Grid>
                      }
                      {
                        ...
                        <Grid
                          ...
                          textAlign={'center'}
                        >
                          {auctionDetails?.vehiclePrice?.currency?.code} {formattedPrice}
                        </Grid>
                      }
                      {
                        auctionDetails?.saleType === 'auction' &&
                        <>
                          ...
                          <Grid item fontWeight={300} fontSize={14} textAlign={'center'}
                            sx={{ textDecoration: 'underline' }}>bid deposit required</Grid>
                        </>
                      }
                    </Grid>
                    {
                      auctionDetails?.highestBidPrice?.amount &&
                      (auctionDetails?.saleType === 'auction' || auctionDetails?.saleOffer?.canAcceptOffers) &&
                      <Grid container flexDirection={'column'} justifyContent={'space-between'}>
                        <Grid
                          mt={2}
                          item
                          ...
                          textAlign={'center'}
                        >
                          Current Highest  {auctionDetails?.saleType === 'sale' ? 'Offer' : 'Bid'}
                        </Grid>
                        <Grid
                          item
                          fontWeight={700}
                          ...
                          textAlign={'center'}
                        >
                          {auctionDetails?.vehiclePrice?.currency?.code} {auctionDetails?.highestBidPrice?.amount}
                        </Grid>
                      </Grid>
                    }
                  </Grid>
...

                  {
                    ...
                    <Grid item textAlign={'center'} py={1} onClick={handleBuyNow}>
                      <CustomButton
                        variant={'contained'}
                        sx={{
                          fontWeight: 800,
                      ...
                      <Grid container alignItems={'flex-end'} justifyContent={'space-between'} width='100%' >
                        ...
                        <Grid item xs={6}>
                          <Grid item mb={1}>
                            <Typography>
                              Your offer: &nbsp;
                              <Typography fontSize={14} color={'red'} sx={{ display: 'inline-block' }}>
                                {auctionDetails?.saleOffer?.offer?.declined ? 'Declined' : ''}
                              </Typography>
                              <Typography fontWeight={600}>
                                {auctionDetails?.saleOffer?.offer?.amount.currency.code}
                                {auctionDetails?.saleOffer?.offer?.amount.amount}
                              </Typography>
                            </Typography>
                          </Grid>
                            ...
                            <Grid container flexDirection={'column'} >
                              <Grid item mb={1}>
                                <Typography>
                                  ...
                                  <Typography fontWeight={600} >
                                    {auctionDetails?.saleOffer?.offer?.counterOfferAmount?.currency.code}
                                    {auctionDetails?.saleOffer?.offer?.counterOfferAmount?.amount}
                                  </Typography>
                                </Typography>
                              </Grid>
                              <Grid item>
...

                  {
                    ...
                    auctionDetails?.saleOffer?.offer?.state !== 'accepted' &&
                    auctionDetails?.saleType === 'sale' &&
                    <Grid item textAlign={'center'} py={1} onClick={toggleSellerOffers}>
                      <CustomButton
                        variant={'contained'}
                        sx={{
                          fontWeight: 800,
        ...
        <CustomDialog
          ...
          component={
            topUpSucces ?
              <Congrats
                title={`${auctionDetails?.saleType === 'sale' ? 'Offer' : 'Bid'} placed successfuly!`}
                type={auctionDetails?.saleType}
              /> :
              <TopUp
...
export const getServerSideProps = async ({ req, res, query }) => {
  ...
  const auctionDetails = await fetchApi({ url: `auction-vehicles/${auctionId}`, headers });
  const auctionTypes = await fetchApi({ url: `auction-vehicle-types`, headers });
