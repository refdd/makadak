...
const ProductListHeader = ({ title, titleLink = '/', seeAll = false, filter, isCategory, type }) => {
    const router = useRouter();
    const dispatch = useDispatch();
    const handleSeeAllClick = () => {
        if (isCategory) {
            dispatch(onAddFilter({ saleType: type, auctionVehicleTypeId: filter.catId, type: filter.sortBy === 'popular' ? 'most-popular' : filter.sortBy }));
            router.push('/search')
        } else router.push(titleLink)
    }
    return (
        <Grid container flex alignItems={'self-end'} padding={3}>
            <Typography color='secondary' fontWeight={900} fontSize={22} style={{ marginRight: 4, color: 'white' }} sx={{ textTransform: 'uppercase' }}>{title}</Typography>
